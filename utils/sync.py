...
def sync_to(cfg):
    cmd = f"gsutil -m rsync -r gs://{cfg.gcloud.bucket}/{cfg.gcloud.work_dir} {cfg.host.work_dir}"
    print(f"Running {cmd}")
    subprocess.run(cmd, shell=True, check=True)

def sync_from(cfg):
    cmd = f"gsutil -m rsync -r {cfg.host.work_dir} gs://{cfg.gcloud.bucket}/{cfg.gcloud.work_dir}"
    print(f"Running {cmd}")
    subprocess.run(cmd, shell=True, check=True)
