...
if __name__ == "__main__":
    ...
    out_folder = cfg["docked_folder"] + "/val_screens"
    os.makedirs(out_folder, exist_ok=True)

    for screen_csv in glob(cfg["bigbind_folder"] + "/val_screens/*.csv"):
        screen_df = pd.read_csv(screen_csv)
        pocket = screen_df.pocket[0]
        print(f"Running vina on {pocket}")
        # out_file = out_folder + "/" + pocket + "_vina_scores.csv"
        ...
        #     scores.append(get_vina_score(cfg, i, row))

        # screen_df["vina_score"] = scores
